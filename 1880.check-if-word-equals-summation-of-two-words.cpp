...
class Solution {
  public:
    bool isSumEqual(string firstWord, string secondWord, string targetWord) {
        auto modify = [](char &a) { a += '0' - 'a'; };
        for_each(firstWord.begin(), firstWord.end(), modify);
        for_each(secondWord.begin(), secondWord.end(), modify);
        for_each(targetWord.begin(), targetWord.end(), modify);
