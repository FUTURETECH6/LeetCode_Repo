...
class Twitter {
  private:
    deque<pair<int, int>> tweets;                       // {user, {tweets}}
    unordered_map<int, unordered_set<int>> followList;  // {follower, {followee}}

  public:
    Twitter() {}

    void postTweet(int userId, int tweetId) { tweets.push_front({userId, tweetId}); }

    vector<int> getNewsFeed(int userId) {
        vector<int> result;
        unsigned count = 1;

        for (auto &[tweetUser, tweetID] : tweets) {
            if (tweetUser == userId || followList[userId].count(tweetUser)) {
    ...
    void follow(int followerId, int followeeId) { followList[followerId].insert(followeeId); }

    void unfollow(int followerId, int followeeId) { followList[followerId].erase(followeeId); }
};
