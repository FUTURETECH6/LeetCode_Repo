...
class Solution {
  private:
    bool x_prefer_y_than_z(int x, int y, int z, vector<vector<int>> &preferences) {
        return find(preferences[x].begin(), preferences[x].end(), y) < find(preferences[x].begin(), preferences[x].end(), z);
    }
    ...
    int unhappyFriends(int n, vector<vector<int>> &preferences, vector<vector<int>> &pairs) {
        ...
        for (unsigned i = 0; i < pairs.size(); i++) {
            for (unsigned j = i + 1; j < pairs.size(); j++) {
                auto &a = pairs[i][0], &b = pairs[i][1], &c = pairs[j][0], &d = pairs[j][1];
                // [a, b], [c, d]
                if (x_prefer_y_than_z(a, c, b, preferences) && x_prefer_y_than_z(c, a, d, preferences)) {
                    unhappySet.insert(a);
                    unhappySet.insert(c);
                }
                if (x_prefer_y_than_z(b, c, a, preferences) && x_prefer_y_than_z(c, b, d, preferences)) {
                    unhappySet.insert(b);
                    unhappySet.insert(c);
                }
                if (x_prefer_y_than_z(a, d, b, preferences) && x_prefer_y_than_z(d, a, c, preferences)) {
                    unhappySet.insert(a);
                    unhappySet.insert(d);
                }
                if (x_prefer_y_than_z(b, d, a, preferences) && x_prefer_y_than_z(d, b, c, preferences)) {
                    unhappySet.insert(b);
                    unhappySet.insert(d);
                }
            }
