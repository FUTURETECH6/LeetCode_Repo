/*
 * @lc app=leetcode id=2191 lang=rust
 *
    ...
    pub fn sort_jumbled(mapping: Vec<i32>, mut nums: Vec<i32>) -> Vec<i32> {
        nums.sort_by_key(|a| {
            ...
            for c in a.to_string().chars() {
                s.push((mapping[c as usize - '0' as usize] as u8 + '0' as u8) as char);
            }
...
pub fn main() {
    println!("{:?}", Solution::sort_jumbled(vec![8, 9, 4, 0, 2, 1, 3, 5, 7, 6], vec![991, 338, 38]));
    ()
}
