...
class Solution:
    def findSubstring(self, s: str, words):
        result = []
        targetLen = sum(len(_) for _ in words)
        ...
        for firInd in range(0, len(s) - targetLen + 1):
            secInd = firInd
            tmpDict = hotDict.copy()
            unqWord = list(tmpDict.keys())
            for _ in range(len(words)):
                for iWord in range(len(unqWord)):
                    if tmpDict[unqWord[iWord]] > 0 and unqWord[iWord] == s[secInd:secInd+len(unqWord[iWord])]:
                        tmpDict[unqWord[iWord]] -= 1
                        secInd += len(words[iWord])
                        break
                else:  # Found no match
                    break
            else:  # No break, so this is a match
                result.append(firInd)
