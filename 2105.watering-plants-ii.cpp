/*
 * @lc app=leetcode id=2105 lang=cpp
 *
 ...
 * water it. Otherwise, they first refill their can (instantaneously) then
 * water the plant.
...
class Solution {
  private:
    ...
    inline void useA() {
        if (sizeA - pPlants->at(posA) < 0) {
            sizeA = capA;
            count++;
        }
        sizeA -= pPlants->at(posA);
    }

    inline void useB() {
        if (sizeB - pPlants->at(posB) < 0) {
            sizeB = capB;
            count++;
        }
        sizeB -= pPlants->at(posB);
    }
